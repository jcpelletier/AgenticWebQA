...
PROFILE_LOCATION_US_LOG = "profilelocationus_run.log"
...
def profile_location_us_flow_prompt() -> str:
    return (
        "1. Register an account\n"
        "2. Open Profile from the header.\n"
        "3. Set Country to United States of America.\n"
        "4. Set State to California and save.\n"
        "5. Refresh the page."
    )


def profile_location_us_flow_success() -> str:
    return (
        "The user is on the Profile page with Country set to United States of America "
        "and State set to California after refreshing."
    )


def run_profile_location_us_integration(
    ...
    cmd = [
        ...
        str(script),
        "--prompt",
        profile_location_us_flow_prompt(),
        "--visual-llm-success",
        profile_location_us_flow_success(),
        "--start-url",
        START_URL,
        "--actions",
        "register_account,profile_open,profile_location_uscalifornia",
        "--max-steps",
        "20",
        "--headless",
        "--verbose",
    ...
    print("== Run Profile Location US California Flow ==")
...
def main() -> int:
    ...
    profile_location_us_log = repo_root / PROFILE_LOCATION_US_LOG
    ...
    print("== Prepare profile location US smoke artifacts ==")
    reset_dir(agent_view_dir)
    profile_location_us_log = pick_writable_log_path(profile_location_us_log)
    ...
    try:
        ...
        if not feature_found:
            message = "Profile location feature not detected in test-site; skipping location US smoke."
            if args.require_feature:
                raise RuntimeError(message)
            print(f"== {message} ==")
            with profile_location_us_log.open("w", encoding="utf-8") as f:
                f.write("SKIP: PROFILE_LOCATION_FEATURE_MISSING\n")
                f.write("FINAL: PASS\n")
            return 0

        run_profile_location_us_integration(
            repo_root, model=args.model, log_path=profile_location_us_log
        )
        print("Local testprofilelocationus sequence passed.")
        return 0
    finally:
        server.terminate()
        try:
