#!/usr/bin/env python
"""
Activity Feed — AgenticWebQA smoke test.

Wipes saved model actions and site hints for 127.0.0.1:8000, then runs the
activity feed flow twice: run 1 learns the actions, run 2 verifies no LLM fallback.

Usage:
    python webtests/run_testactivityfeed_local.py [--skip-install] [--no-wipe]
...
TEST_CASES = [
    (
        ...
        "You are on the Home page and see 'Welcome, demo'.",
        "login_demo",
        DEFAULT_MODEL,
    ),
    (
        ...
        (
            "1. Log in with username 'demo' and password 'demo123'.\n"
            "2. On the home page, find the Activity Feed section below the welcome text.\n"
            "3. Click the message input field, type 'Hello {rand_string}', and click the Post button.\n"
            "4. Confirm 'Hello {rand_string}' appears in the feed with a Delete button visible."
        ),
        "The Activity Feed shows a post with text 'Hello {rand_string}' and a Delete button is visible next to it.",
        "login_demo,activity_feed",
        DEFAULT_MODEL,
    ),
]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Activity Feed AgenticWebQA smoke test"
    )
    ...
    try:
        ...
        print(
            f"\nAll {len(TEST_CASES)} activity feed smoke test(s) passed (both runs)."
        )
        return 0
