---
description: "Turn a product requirement into an actionable feature TDD document via a planning-orchestrator workflow. Use when starting a new feature or a meaningful behavior change."
---
...
## Planning Authority
- `the planning skill` is the top authority for planning-phase orchestration.
- Planning is the manager role. It owns requirement intake, subagent delegation, and final TDD authorship.
- Planning must call `the qa-manager skill` as a subagent for test strategy and required test commands.
- Planning must call a traceability/requirements-validation subagent (default: `the qa-traceability skill`) for REQ/AC mapping completeness.
- Planning does not directly orchestrate `QA_WebAutomation` or `QA_API`; those are routed by `QAManager` as needed.

## Source Of Truth Documents
- Your project's architecture and coding guidelines docs (e.g. `AGENTS.md`, `docs/Code_Style.md`,
  `docs/Structure.md`, or equivalents — substitute your project's actual files).
- `the planning skill`: planning-phase orchestration rules and output contract.
- `the qa-manager skill`: QA orchestration workflow, report contract, and completion gates.
- `the qa-traceability skill`: requirement-to-test traceability and evidence policy.
- `the qa-web-automation skill`: subordinate QA specialization (invoked by `QAManager` when applicable).
- `the qa-api skill`: subordinate QA specialization (invoked by `QAManager` when API scope exists).

## Inputs Required
- Product requirement text (problem, scope, constraints).
- Relevant existing feature docs (e.g. under `docs/features/`).
...
Planner output must:
- Create exactly one file at `docs/features/tdd_<FeatureSlug>.md` (or your project's equivalent path).
...
- Use portable command references in testing instructions (e.g. `agenticwebqa:run_test(name="<feature>")` or `python -m agenticwebqa --start-url ... --model <model>`).
...
- `QAManager` (required):
  - Returns test strategy, test IDs, required commands, and QA risks using `the qa-manager skill` output contract.
...
- `RequirementsTraceabilityReviewer` (required):
  - Default implementation: `the qa-traceability skill`.
...
11. Create a linear checklist with implementation + validation + docs steps. Default precommit policy: do not add new feature smoke tests to your precommit suite unless explicitly requested by a developer.
12. Add three separate checklist items for test execution:
   - run your precommit suite
   - run new feature unit tests
   - run new feature integration/smoke tests
   and capture pass evidence in the TDD.
13. If new automated unit/e2e coverage is introduced, add a checklist step to update your release suite coverage.
...
- Checklist/test-plan language must default to NO automatic precommit suite edits unless explicitly requested by a developer.
...
- Must include a checklist step to update your release suite when new automated unit/e2e coverage is introduced.
...
- Smoke command references should use MCP tool form (`agenticwebqa:run_test(name="<feature>", model="<model>")`)
  or portable CLI form (`python -m agenticwebqa --start-url <url> ... --model <model>`).
...
Use your project's architecture, structure, and coding-style docs as constraints.
...
1) Invoke QAManager (the qa-manager skill) as a subagent and capture its returned test strategy, commands, risks, and verdict.
2) Invoke a traceability reviewer subagent (default the qa-traceability skill) and capture REQ/TEST/AC mapping audit output.
...
Default to no precommit suite changes unless explicitly requested by a developer.
Require checklist items to separately run your precommit suite, feature unit tests, and feature integration/smoke tests, and to record which test commands passed in the TDD.
Require generated feature integration/smoke command examples to include `--model <model>` (or `model=` in MCP tool form) unless a different model is explicitly requested.
Require a checklist step to update your release suite whenever new automated unit/e2e coverage is introduced.
...
- Feature TDD exists in the agreed docs location.
...
- TDD includes explicit, separate validation tasks to execute your precommit suite, feature unit tests, and feature integration/smoke tests.
- TDD includes a step to update your release suite when feature scope introduces new automated unit/e2e coverage.
